...
# rise (if applicable)
## 1.18
execute unless score legacy global matches 1.. as @e[tag=riser] at @s if score rise_time time = rising_time time unless score @s y matches 310.. if score height_limit global matches 1 run function lavarising:rise
execute unless score legacy global matches 1.. as @e[tag=riser] at @s if score time time = rising_time time unless score @s y matches 310.. if score height_limit global matches 1 run function lavarising:rise
## 1.17
execute if score legacy global matches 1.. as @e[tag=riser] at @s if score rise_time time = rising_time time unless score @s y matches 245.. if score height_limit global matches 1 run function lavarising:rise
execute if score legacy global matches 1.. as @e[tag=riser] at @s if score time time = rising_time time unless score @s y matches 245.. if score height_limit global matches 1 run function lavarising:rise

# quick note ^ i have no clue why this code is so scuffed - 22/06/2022
...
## 1.18
execute unless score legacy global matches 1.. if score period time matches 2 run bossbar set lavarising:period name [{"text":"The lava is currently at Y: ","color":"red"},{"score":{"name":"@e[tag=riser,limit=1]","objective":"y"},"color":"yellow","bold":true},{"text":"/310","color":"gold"}]
## 1.18
execute if score legacy global matches 1.. if score period time matches 2 run bossbar set lavarising:period name [{"text":"The lava is currently at Y: ","color":"red"},{"score":{"name":"@e[tag=riser,limit=1]","objective":"y"},"color":"yellow","bold":true},{"text":"/245","color":"gold"}]
